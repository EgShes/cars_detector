...
class Processing:
    ...
    @staticmethod
    def correct_bbox(bbox: Tuple[float, float, float, float], transform_info: Dict[str, Any]) -> \
...
def load_model(backbone: str = 'resnet50', checkpoint_path: Optional[str] = '/tmp/cars_detection.pth', 
    device: torch.device = config.device) -> SSD300:
...
@torch.no_grad()
def run_network(model: SSD300, input: torch.Tensor, transform_info: Dict[str, Any], threshold: float = 0.6) -> torch.Tensor:
